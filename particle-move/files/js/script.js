...
utils_RendererManager.init = function() {
	...
	utils_RendererManager._parent.setClearColor(4112,1);
...
view_ObjectManager.create = function() {
	view_object_Particle.create();
	utils_EventManager.trigger("objectCreated");
};
view_ObjectManager.onUpdate = function() {
	view_object_Particle.onUpdate();
};
...
view_object_Particle.create = function() {
	var material = new THREE.PointsMaterial({ size : 1, sizeAttenuation : false, transparent : true, opacity : 1, vertexColors : THREE.VertexColors});
	var geometry = view_object_Particle.getGeometry();
	view_object_Particle._object = new THREE.Points(geometry,material);
	utils_SceneManager.add(view_object_Particle._object);
};
view_object_Particle.onUpdate = function() {
	...
	var vertices = view_object_Particle._object.geometry.vertices;
	var _g = 0;
	while(_g < vertices.length) {
		var vertice = vertices[_g];
		++_g;
		view_object_Particle.updateParticle(vertice);
	}
};
view_object_Particle.getGeometry = function() {
	...
	while(_g < 2000) {
		...
		var z = jp_okawa_utils_MathTools.randomInt(-10,10);
		var particle = new THREE.Vector3(x,y,z);
		particle.xSpeed = jp_okawa_utils_MathTools.randomFloat(.5,3);
		particle.ySpeed = jp_okawa_utils_MathTools.randomFloat(.5,1);
		particle.maxY = jp_okawa_utils_MathTools.randomFloatSpread(20) + y;
		geometry.vertices.push(particle);
		var r = jp_okawa_utils_MathTools.randomInt(0,100);
		var g = jp_okawa_utils_MathTools.randomInt(30,255);
		var b = jp_okawa_utils_MathTools.randomInt(0,20);
		geometry.colors.push(new THREE.Color("rgb(" + r + "," + g + "," + b + ")"));
	}
	return geometry;
};
view_object_Particle.updateParticle = function(particle) {
	particle.x += particle.xSpeed;
	particle.y += particle.ySpeed;
	if(particle.maxY <= particle.y || -particle.maxY >= particle.y) {
		particle.ySpeed *= -1;
	}
	if(400. <= particle.x) {
		particle.x = -400.;
...
view_object_Particle.VIEW_WIDTH = 800;
view_object_Particle.VIEW_HEIGHT = 800;
view_object_Particle.LENGTH = 2000;
Main.main();
