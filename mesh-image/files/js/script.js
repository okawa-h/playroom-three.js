...
object_Viewer.create = function() {
	...
	object_Viewer._meshData = [];
	var data = utils_MaterialManager.get("pikachu");
	object_Viewer.createMesh(data.width,data.height,data.pixelData);
	object_ViewerGroup.add(object_Viewer._parent);
};
object_Viewer.onUpdate = function() {
};
object_Viewer.createMesh = function(width,height,pixels) {
	var pixelCounter = 0;
	var halfW = width * .5;
	var halfH = height * .5;
	var _g1 = 0;
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			var l = _g3++;
			var r = pixels[pixelCounter];
			var g = pixels[pixelCounter + 1];
			var b = pixels[pixelCounter + 2];
			var opacity = pixels[pixelCounter + 3];
			pixelCounter = i * 4 + l * (4 * width);
			var geometry = new THREE.SphereGeometry(10);
			var material = new THREE.MeshBasicMaterial({ side : THREE.FrontSide, transparent : true, opacity : opacity});
			material.color = new THREE.Color("rgb(" + r + "," + g + "," + b + ")");
			var mesh = [new THREE.Mesh(geometry,material)];
			mesh[0].position.x = 0;
			mesh[0].position.y = 0;
			mesh[0].position.z = 0;
			object_Viewer._parent.add(mesh[0]);
			TweenMax.to(mesh[0].position,1,{ x : (i - halfW) * 10, y : -(l - halfH) * 10, z : jp_okawa_utils_MathTools.randomFloatSpread(0), ease : Expo.easeInOut, onComplete : (function(mesh1) {
				return function() {
					object_Viewer.animate(mesh1[0].position);
				};
			})(mesh)});
		}
	}
};
object_Viewer.animate = function(position) {
	var x = position.x;
	var y = position.y;
	TweenMax.to(position,1,{ x : x + jp_okawa_utils_MathTools.randomFloatSpread(20), y : y + jp_okawa_utils_MathTools.randomFloatSpread(20), ease : Sine.easeIn, repeat : -1, yoyo : true, onComplete : function() {
	}});
};
...
object_ViewerGroup.remove = function(obj) {
	object_ViewerGroup._parent.remove(obj);
};
...
utils_materialManager_Order.loadVideo = function(num,data) {
	return new Promise(function(resolve,reject) {
		...
		video.oncanplaythrough = function() {
			...
			utils_materialManager_Data.set(data1,data2,{ video : video, ctx : tmp, texture : new THREE.Texture(canvas)});
...
view_Camera.init = function() {
	...
	view_Camera._camera.position.set(200,200,1800);
...
object_Viewer.BOX_SIZE = 10;
utils_Helper.ON_HELPER = false;
utils_Helper.ON_AXIS = true;
utils_Helper.ON_GRID = false;
utils_MaterialManager._orderList = [{ id : "pokemon", src : "files/img/pokemon.png", type : "image"},{ id : "houou", src : "files/img/houou.png", type : "image"},{ id : "kabigon", src : "files/img/kabigon.png", type : "image"},{ id : "pikachu", src : "files/img/pikachu.png", type : "image"}];
view_Camera.FOV = 60;
view_Camera.NEAR = 1;
