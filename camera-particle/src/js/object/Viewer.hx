...
class Viewer {
	...
	public static function create():Void {
		...
		var data : VideoMaterial = CameraManager.get();
		_canvas = data.canvas;
		_ctx    = data.ctx;
		_video  = data.video;
		...
		public static function onUpdate():Void {
			...
			for (i in 0 ... width) {
				for (l in 0 ... height) {
					...
					_particlePositions[ posi++ ] = x + MathTools.randomFloatSpread(10);
					_particlePositions[ posi++ ] = y + MathTools.randomFloatSpread(10);
					_particlePositions[ posi++ ] = MathTools.randomFloatSpread(200);
					length++;
