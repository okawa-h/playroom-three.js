...
object_Viewer.create = function() {
	...
	var geometry = new THREE.PlaneGeometry(object_Viewer._video.videoWidth,object_Viewer._video.videoHeight);
	var material = new THREE.MeshBasicMaterial({ side : THREE.DoubleSide, map : object_Viewer._texture});
	object_Viewer._video.loop = true;
	object_Viewer._video.play();
	object_Viewer._parent = new THREE.Mesh(geometry,material);
	object_Viewer._parent.position.set(0,0,0);
	object_ViewerGroup.add(object_Viewer._parent);
};
object_Viewer.onUpdate = function() {
	var timer = new Date().getTime() * .001;
	object_Viewer._parent.rotation.y = timer * .2;
