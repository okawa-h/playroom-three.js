...
Main.init = function(event) {
	view_Window.init();
	view_Window.setEvent({ "audioLoaded" : Main.setup});
	utils_AudioManager.load();
};
Main.setup = function() {
...
var utils_AudioManager = function() { };
utils_AudioManager.__name__ = true;
utils_AudioManager.load = function() {
	utils_AudioManager._jText = $("#load");
	utils_AudioManager._materialData = new haxe_ds_StringMap();
	utils_AudioManager.promise();
};
utils_AudioManager.promise = function() {
	var length = utils_AudioManager._manifest.length;
	var counter = 0;
	var loadAudio = function(num) {
		return new Promise(function(resolve,reject) {
			var data = utils_AudioManager._manifest[num];
			var request = new XMLHttpRequest();
			request.open("GET","files/mp3/" + Std.string(data.src),true);
			request.responseType = "arraybuffer";
			request.onload = function() {
				var ctx = new AudioContext();
				ctx.decodeAudioData(request.response);
				var k = data.id;
				var v = { src : data.src, ctx : ctx};
				var _this = utils_AudioManager._materialData;
	...
	while(_g1 < _g) {
		...
		promise = promise.then((function(i1) {
			return function(src) {
				return loadAudio(i1[0]);
			};
		})(i));
	}
	promise = promise.then(function(num1) {
		utils_AudioManager.onLoaded();
	});
	promise = promise["catch"](function(reason) {
		console.log(reason);
	});
};
utils_AudioManager.onLoaded = function() {
	utils_AudioManager._jText.fadeOut(200);
	view_Window.trigger("audioLoaded");
};
utils_AudioManager.onProgress = function(current,length) {
	var progress = Math.floor(current / length * 100);
	utils_AudioManager._jText.text("Loading... " + progress + "%");
};
utils_AudioManager.getItem = function(id) {
	var _this = utils_AudioManager._materialData;
	if(__map_reserved[id] != null ? _this.existsReserved(id) : _this.h.hasOwnProperty(id)) {
		var _this1 = utils_AudioManager._materialData;
...
object_Particle.WIDTH = 1000;
object_Particle.HEIGHT = 1000;
utils_AudioManager.BASE_PATH = "files/mp3/";
utils_AudioManager._manifest = [{ id : "bgm", src : "toikioku_healing.mp3"}];
