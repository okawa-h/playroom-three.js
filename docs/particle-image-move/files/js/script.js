...
var jp_okawa_utils_ArrayTools = function() { };
jp_okawa_utils_ArrayTools.__name__ = true;
jp_okawa_utils_ArrayTools.getRandomItem = function(array) {
...
object_Particle.create = function() {
	var material = new THREE.PointsMaterial({ size : 5, sizeAttenuation : false, transparent : true, opacity : .8, vertexColors : THREE.VertexColors});
...
object_Particle.getGeometry = function() {
	var materialData = utils_MaterialManager.getItem("image");
	...
	var easeArray_0 = Expo.easeOut;
	var easeArray_1 = Circ.easeOut;
	var easeArray_2 = Sine.easeOut;
	var _g1 = 0;
	var _g = imageW;
	while(_g1 < _g) {
		var i = _g1++;
		...
		while(_g3 < _g2) {
			...
			if(!is_trans) {
				var x = i - imageW * .5;
				var y = -(l - imageH * .5);
				var rz = simplexNoise.noise(x / 30,y / 30);
				var particle = new THREE.Vector3(x,y,rz);
				particle.animation = TweenMax.to(particle,.5,{ z : jp_okawa_utils_MathTools.randomFloatSpread(20), ease : Elastic.easeInOut, paused : true, repeat : -1, yoyo : true});
...
view_Camera.init = function() {
	...
	view_Camera._camera.position.set(0,-600,150);
	view_Camera._camera.lookAt(new THREE.Vector3(0,0,0));
	utils_SceneManager.add(view_Camera._camera);
	view_Camera.animation();
};
view_Camera.animation = function() {
	TweenMax.to(view_Camera._camera.position,1,{ z : 0, ease : Expo.easeInOut, delay : 1, onComplete : function() {
		TweenMax.to(view_Camera._camera.position,5,{ y : 600, ease : Expo.easeInOut, onComplete : function() {
			TweenMax.to(view_Camera._camera.position,3,{ y : 10, ease : Expo.easeInOut, onComplete : function() {
				TweenMax.to(view_Camera._camera.position,2,{ z : 600, ease : Expo.easeInOut});
			}});
		}});
	}});
};
view_Camera.onUpdate = function(mouseX,mouseY) {
	view_Camera._camera.lookAt(utils_SceneManager.getPosition());
...
object_Particle.INTERVAL = 1;
object_Particle.NOIZE_X = 30;
object_Particle.NOIZE_Y = 30;
