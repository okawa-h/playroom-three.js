...
Main.init = function(event) {
	...
	window.document.getElementById("stage").appendChild(utils_RendererManager.getElement());
	Main.start();
};
Main.start = function() {
	view_Camera.init();
	view_Light.init();
	view_ObjectManager.init();
...
utils_EventManager.setEvent = function() {
	utils_EventManager._window.addEventListener("mousemove",utils_EventManager.onMousemove);
	utils_EventManager._window.addEventListener("resize",utils_EventManager.onResize);
	utils_EventManager.trigger("resize");
};
utils_EventManager.trigger = function(eventName) {
	var event = window.document.createEvent("HTMLEvents");
	event.initEvent(eventName,true,true);
	utils_EventManager._window.dispatchEvent(event);
};
...
utils_EventManager.width = function() {
	return utils_EventManager._window.innerWidth;
};
utils_EventManager.height = function() {
	return utils_EventManager._window.innerHeight;
};
...
utils_RendererManager.init = function() {
	...
	utils_RendererManager._parent.setClearColor(90252);
};
...
view_Camera.init = function() {
	...
	view_Camera._camera.position.set(400,400,700);
...
var view_ObjectManager = function() { };
view_ObjectManager.__name__ = true;
view_ObjectManager.init = function() {
	var geometry = new THREE.Geometry();
	var _g = 0;
	while(_g < 500000) {
		var i = _g++;
		geometry.vertices.push(view_object_Particle.create());
	}
	var material = new THREE.PointsMaterial({ color : 90252, size : 2});
	view_ObjectManager._objects = new THREE.Points(geometry,material);
	utils_SceneManager.add(view_ObjectManager._objects);
};
view_ObjectManager.onUpdate = function() {
	view_ObjectManager._objects.rotation.y += .002;
	var vertices = view_ObjectManager._objects.geometry.vertices;
	var _g = 0;
	while(_g < 500000) {
		var i = _g++;
		var particle = vertices[i];
		view_object_Particle.onUpdate(particle);
	}
	view_ObjectManager._objects.geometry.verticesNeedUpdate = true;
};
...
view_object_Particle.onUpdate = function(target) {
	target.x += target.velocity.x * 1;
	target.y += target.velocity.y * 1;
	target.z += target.velocity.z * 1;
...
view_ObjectManager.LENGTH = 500000;
view_object_Particle.BOX_WIDTH = 500;
view_object_Particle.BOX_HEIGHT = 500;
view_object_Particle.BOX_DEPTH = 500;
view_object_Particle.SPEED = 1;
Main.main();
